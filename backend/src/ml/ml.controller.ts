 ...
 * Detection, and raises an Alert for anything high/critical.
 *
...
export class MlController {
  ...
  async ingest(@Param("structureId") structureId: string, @UploadedFile() file?: Express.Multer.File) {
    ...
    const structure = this.structures.findOne(structureId); // throws 404 if unknown
    const result = await this.ml.predict(file.buffer, file.originalname);

    const created: Detection[] = result.predictions.map((p) => {
      ...
      this.detections.create(detection);

      if (HIGH_SEVERITY.includes(detection.severity)) {
        this.structures.bumpRisk(structureId, detection.severity);
        this.alerts.raise({
          id: randomUUID(),
          structureId,
